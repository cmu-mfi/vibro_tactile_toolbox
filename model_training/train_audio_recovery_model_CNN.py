...
class VibrotactileDataset(Dataset):
  ...
  def __init__(self, dataset_type, channels):
    ...
    if dataset_type == 'lego':
        ...
        paths = glob.glob('/mnt/hdd1/vibrotactile_data/nist_dataset/*/dsub/old_vel*/MoveDown/fail/audio/*.png')
        paths += glob.glob('/mnt/hdd1/vibrotactile_data/nist_dataset/*/dsub/vel*/MoveDown/fail/audio/*.png')
        paths = [path for path in paths if 'failure' not in path]
    elif dataset_type == 'waterproof':
        paths = glob.glob('/mnt/hdd1/vibrotactile_data/nist_dataset/*/waterproof/*/MoveDown/fail/audio/*.png')
        paths = [path for path in paths if 'failure' not in path]
    ...
    for path in paths:
      ...
      if current_num % 4 == 0:
          ...
          x_perturb = pd.to_numeric(perturbs[:perturbs.find('_')])
          perturbs = perturbs[perturbs.find('_')+1:]
          y_perturb = pd.to_numeric(perturbs[:perturbs.find('_')])
          perturbs = perturbs[perturbs.find('_')+1:]
          theta_perturb = pd.to_numeric(perturbs[:perturbs.find('_')])
